...
print("=" * 80)
print("DUCKDB IPv4 MERGE (PRESERVING ALL FIELD RULES)")
print("=" * 80)
...
print(f"✓ FILE 1 Processed: {f1_count:,} records")
...
print(f"✓ FILE 2 Processed: {f2_count:,} records")
print(f"Time: {time.time() - t1:.2f} sec")

# =========================================================
# STEP 3: HIGH-SPEED CORRELATION MERGE (SLIDING MINUTE WINDOW)
# =========================================================
print("\n[3/5] Correlating IP addresses (Preserving All Fields)...")
t1 = time.time()

# Keeping ALL your field matching rules perfectly identical.
# The ONLY optimization is changing '=' to 'BETWEEN -1 AND +1' 
# to capture traffic that crossed over a minute boundary.
merge_query = """
    ...
    LEFT JOIN f1_distinct f1 ON 
        f1.START_TIME_MIN BETWEEN (f2.START_TIME_MIN - 1) AND (f2.START_TIME_MIN + 1) AND
...
# Safely select the absolute best time match if a record overlaps across buckets
con.execute("""
